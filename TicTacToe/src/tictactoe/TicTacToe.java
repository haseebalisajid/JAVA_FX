...
class GameControls extends HBox {
  GameControls(final GameManager gameManager, final Game game) {
    ...
    playAgainButton.setOnAction((ActionEvent actionEvent) -> {
        gameManager.newGame();
    });
    ...
    exitButton.setOnAction((ActionEvent actionEvent) -> {
...
final class Game {
  ...
  private final ReadOnlyObjectWrapper<Square.State> currentPlayer = new ReadOnlyObjectWrapper<>(Square.State.CROSS);
  ...
  private final ReadOnlyObjectWrapper<Square.State> winner = new ReadOnlyObjectWrapper<>(Square.State.EMPTY);
...
class SquareSkin extends StackPane {
  ...
  SquareSkin(final Square square) {
    ...
    square.stateProperty().addListener((ObservableValue<? extends Square.State> observableValue, Square.State oldState, Square.State state) -> {
        switch (state) {
